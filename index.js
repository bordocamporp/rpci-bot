...
// ====== COSTANTI ======
const GAME_ROLES = ['POR', 'TD', 'DC', 'TS', 'CDC', 'CC', 'COC', 'ES', 'ED', 'AS', 'AD', 'ATT'];
const ROLE_LABELS = {
  POR: 'Portiere',
  TD: 'Terzino Destro',
...
function parseSettings(type, raw) {
  ...
  if (type === 'european_cup') {
    settings.gironi = Number(settings.gironi || 4);
  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === 'sorteggia_squadre') {
        ...
        await supabase.from('draft_assignments').delete().neq('id', '00000000-0000-0000-0000-000000000000').catch(() => null);
        await supabase.from('draft_teams').delete().neq('id', '00000000-0000-0000-0000-000000000000').catch(() => null);
      ...
      if (interaction.commandName === 'crea_competizione') {
        ...
        return interaction.reply({ content: '🏆 Seleziona tipo competizione.', components: [buildCompetitionTypeSelect()], flags: MessageFlags.Ephemeral });
      }
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId === 'player_signup_data_modal') {
        ...
        if (!Number.isInteger(overall) || overall < 1 || overall > 99) return interaction.reply({ content: '❌ Overall non valido.', flags: MessageFlags.Ephemeral });
        ...
        if (member) {
          ...
          const role = await findDiscordRoleForGameRole(interaction.guild, draft.primary_role);
