...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .setDescription('Pubblica il pannello player cerca squadra'),
...
function buildPlayerLookingPanel() {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      '• piattaforma\n' +
      '• ID console\n' +
      '• esperienze e competizioni disputate\n\n' +
  ...
  const button = new ButtonBuilder()
    .setCustomId('player_looking_create')
...
function buildPlayerLookingEmbed(data, user) {
  return new EmbedBuilder()
    .setColor(0x3498db)
    .setTitle('🔎 PLAYER CERCA SQUADRA')
    .setDescription('Un player ha pubblicato il proprio profilo per trovare squadra su RPCI.')
    .addFields(
      { name: '👤 Giocatore', value: data.playerName, inline: true },
      { name: '🏷️ Discord', value: `<@${user.id}>`, inline: true },
      { name: '🎂 Età', value: String(data.age), inline: true },
      { name: '⚽ Ruolo / ID Console', value: data.roleConsole, inline: false },
      { name: '🎮 Piattaforma', value: data.platform, inline: true },
      { name: '📝 Esperienze', value: data.experiences.slice(0, 1024), inline: false }
    )
    .setFooter({ text: 'RPCI • Player cerca squadra' })
    .setTimestamp();
}
...
const GAME_ROLES = [
  ...
  'ATT'
    .addStringOption(option =>
      option
        .setName('esperienze')
        .setDescription('Esperienze, club precedenti e competizioni disputate')
        .setRequired(true)
    )
];
  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === 'pannello_cerco_squadra') {
        ...
        if (!isStaffMember(member)) {
          return interaction.reply({
            content: '❌ Solo lo staff può pubblicare il pannello Player Cerca Squadra.',
            flags: MessageFlags.Ephemeral
          });
        }

        const channel = await interaction.guild.channels.fetch(PLAYER_LOOKING_CHANNEL_ID).catch(() => null);
        if (!channel) {
          return interaction.reply({
            content: '❌ Canale Player Cerca Squadra non trovato.',
            flags: MessageFlags.Ephemeral
          });
        }

        await channel.send(buildPlayerLookingPanel());

        return interaction.reply({
          content: `✅ Pannello Player Cerca Squadra pubblicato in <#${PLAYER_LOOKING_CHANNEL_ID}>.`,
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (interaction.commandName === 'cerco_squadra') {
        return interaction.reply({
          content: '🔎 Usa il pulsante nel canale Player Cerca Squadra per creare il tuo annuncio.',
          flags: MessageFlags.Ephemeral
        });
      }
...
` +
    ...
    if (interaction.isButton()) {
      if (interaction.customId === 'player_looking_create') {
        if (interaction.channelId !== PLAYER_LOOKING_CHANNEL_ID) {
          return interaction.reply({
            content: '❌ Puoi usare questo pulsante solo nel canale Player Cerca Squadra.',
            flags: MessageFlags.Ephemeral
          });
        }

        const modal = new ModalBuilder()
          .setCustomId('player_looking_modal')
        ...
        const age = new TextInputBuilder()
          .setCustomId('age')
          .setLabel('ETÀ')
          .setPlaceholder('Scrivi solo il numero, senza limiti massimi')
          .setStyle(TextInputStyle.Short)
          .setRequired(true);
        ...
        const roleConsole = new TextInputBuilder()
          .setCustomId('role_console')
          .setLabel('RUOLO E ID CONSOLE')
          .setPlaceholder('Esempio: ATT - AlexPSN')
          .setStyle(TextInputStyle.Short)
          .setRequired(true);

        const experiences = new TextInputBuilder()
          .setCustomId('experiences')
          .setLabel('ESPERIENZE / COMPETIZIONI')
          .setPlaceholder('Club precedenti, competizioni disputate, obiettivi...')
          .setStyle(TextInputStyle.Paragraph)
          .setRequired(true);
        ...
        modal.addComponents(
          ...
          new ActionRowBuilder().addComponents(roleConsole),
  ...
  status: 'pending'
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId === 'player_looking_modal') {
        ...
        const roleConsole = interaction.fields.getTextInputValue('role_console').trim();
        ...
        await channel.send({
          embeds: [buildPlayerLookingEmbed({ playerName, age, platform, roleConsole, experiences }, interaction.user)]
        });

        return interaction.reply({
          content: '✅ Il tuo annuncio Player Cerca Squadra è stato pubblicato.',
          flags: MessageFlags.Ephemeral
        });
      }
