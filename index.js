...
async function getCurrentPlayerClub(discordId) {
  const { data: playerRows } = await supabase
    .from('club_application_players')
    .select('*, club_applications(id, status, clubs(name))')
...
function buildLookingRoleSelect() {
  return new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .addOptions(GAME_ROLES.map(role => ({ label: role, value: role })))
  );
}
...
` +
    ...
    if (interaction.isButton()) {
      if (interaction.customId.startsWith('fa_contact_')) {
        ...
        return interaction.reply({ content: '✅ Messaggio di contatto inviato al giocatore.', flags: MessageFlags.Ephemeral });
      }
  ...
  status: 'pending'
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId === 'looking_team_modal') {
        ...
        return interaction.reply({ content: '✅ Ora seleziona il tuo ruolo.', components: [buildLookingRoleSelect()], flags: MessageFlags.Ephemeral });
      }
