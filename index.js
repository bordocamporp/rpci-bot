      ...
      .addOptions(
        Array.from({ length: max - min + 1 }, (_, i) => {
          ...
          return {
            label: value,
            value,
            default: String(currentValue) === value
          };
        })
      )
  );
...
function buildCompetitionLevelSelect(draft) {
  return new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .addOptions([
        { label: 'Campionato superiore', value: 'superiore', description: 'Esempio: Serie A', default: draft.settings?.livello === 'superiore' },
        { label: 'Campionato inferiore', value: 'inferiore', description: 'Esempio: Serie B', default: draft.settings?.livello === 'inferiore' },
        { label: 'Campionato unico / non collegato', value: 'unico', description: 'Nessuna promozione-retrocessione collegata', default: !draft.settings?.livello || draft.settings?.livello === 'unico' }
      ])
  );
}
    ...
    new StringSelectMenuBuilder()
      ...
      .addOptions(
        comps.slice(0, 25).map(comp => ({
          ...
          value: comp.id,
          default: draft.settings?.collegata_competizione_id === comp.id
        }))
      )
  );
}
...
function buildCompetitionComponents(draft) {
  ...
  if (draft.type === 'league') {
    components.push(buildNumberSelect('competition_groups_select', 'Numero gironi: 1-10', 1, 10, draft.settings?.gironi ?? 1));
    components.push(buildNumberSelect('competition_promoted_select', 'Promozioni: 0-10', 0, 10, draft.settings?.promosse ?? 0));
    components.push(buildNumberSelect('competition_relegated_select', 'Retrocessioni: 0-10', 0, 10, draft.settings?.retrocesse ?? 0));
    components.push(buildCompetitionLevelSelect(draft));
    const linked = buildLinkedCompetitionSelect(draft);
    if (linked) components.push(linked);
  }
...
` +
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId.startsWith('competition_modal_')) {
        ...
        return interaction.reply({
          content:
            '📎 Ora carica il **logo della competizione** come immagine nel canale.\n\n' +
            'Il messaggio con il logo verrà cancellato automaticamente. Non chiudere questa procedura.',
          flags: MessageFlags.Ephemeral
        });
      }
    }
...
client.on('messageCreate', async message => {
  ...
  const pendingCompetitionDraft = competitionDrafts.get(message.author.id);
  if (pendingCompetitionDraft?.awaitingLogo && message.attachments.size > 0) {
    const file = message.attachments.first();
    await message.delete().catch(() => null);

    const isImage = file.contentType?.startsWith('image/') || /\.(png|jpg|jpeg|webp)$/i.test(file.name || '');
    if (!isImage) {
      competitionDrafts.delete(message.author.id);
      return message.author.send('❌ Il logo deve essere un’immagine. Ricomincia con `/crea_competizione`.').catch(() => null);
    }

    const clubs = await getApprovedClubsWithApplications();
    if (!clubs.length) {
      competitionDrafts.delete(message.author.id);
      return message.author.send('❌ Non ci sono club approvati disponibili.').catch(() => null);
    }

    pendingCompetitionDraft.logoUrl = file.url;
    pendingCompetitionDraft.clubs = clubs;
    pendingCompetitionDraft.existingLeagueCompetitions = pendingCompetitionDraft.type === 'league'
      ? await getExistingLeagueCompetitionsForLink(pendingCompetitionDraft.name)
      : [];
    pendingCompetitionDraft.awaitingLogo = false;

    return message.author.send({
      content: '✅ Logo ricevuto. Ora imposta le opzioni e seleziona le squadre partecipanti.',
      embeds: [buildCompetitionEmbed(pendingCompetitionDraft)],
      components: buildCompetitionComponents(pendingCompetitionDraft)
    }).catch(async () => {
      await message.channel.send({
        content: `<@${message.author.id}> ✅ Logo ricevuto. Ora imposta le opzioni e seleziona le squadre partecipanti.`,
        embeds: [buildCompetitionEmbed(pendingCompetitionDraft)],
        components: buildCompetitionComponents(pendingCompetitionDraft)
      }).catch(() => null);
    });
