...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .setDescription('Staff: pubblica il pannello iscrizione singola player'),

  new SlashCommandBuilder()
    .setName('chiudi_iscrizioni_player')
    .setDescription('Staff: chiude le iscrizioni singole player'),

  new SlashCommandBuilder()
    .setName('sorteggia_squadre')
    .setDescription('Staff: sorteggia le squadre bilanciate per ruolo')
    .addIntegerOption(option =>
      option
        .setName('numero_squadre')
        .setDescription('Numero squadre da creare')
        .setRequired(true)
    )
    .addStringOption(option =>
      option
        .setName('nomi_squadre')
        .setDescription('Nomi squadre separati da virgola. Esempio: Milan, Inter, Juve')
        .setRequired(false)
    )
    .addIntegerOption(option =>
      option
  ...
  new SlashCommandBuilder()
    ...
    .setDescription('Staff: assegna il capitano a una squadra sorteggiata')
    .addUserOption(option =>
      option
        .setName('player')
        .setDescription('Player scelto come capitano')
        .setRequired(true)
    )
    .addStringOption(option =>
      option
        .setName('squadra')
        .setDescription('Nome esatto della squadra sorteggiata')
        .setRequired(true)
    ),
  ...
  new SlashCommandBuilder()
    ...
    .addUserOption(option =>
      option
        .setName('player')
        .setDescription('Player da cui rimuovere capitano')
        .setRequired(true)
    ),
...
// =====================================================
// NUOVA STRUTTURA: ISCRIZIONE PLAYER -> DRAFT -> CAPITANI
// =====================================================
const playerSignupDrafts = new Map();
const ROLE_LABELS = {
  POR: 'Portiere',
  TD: 'Terzino Destro',
...
function buildPlayerSignupPanel() {
  const embed = new EmbedBuilder()
    .setTitle('📝 ISCRIZIONE PLAYER RPCI')
    .setColor(0xd4af37)
    .setDescription(
      'Iscriviti singolarmente alla competizione RPCI.\n\n' +
      'Scegli il tuo ruolo principale, compila i dati e il bot ti assegnerà automaticamente il ruolo **PLAYER** e il ruolo FC scelto.\n\n' +
      'Dopo la chiusura iscrizioni, lo staff effettuerà il sorteggio delle squadre bilanciato per ruolo.'
    )
    .setFooter({ text: 'RPCI • Player Draft System' })
    .setTimestamp();
...
function buildPlayerSignupRoleSelect() {
  return new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      .setCustomId('player_signup_role_select')
      .setPlaceholder('Seleziona il tuo ruolo principale FC')
      .addOptions(GAME_ROLES.map(role => ({
        label: `${role} • ${ROLE_LABELS[role] || role}`.slice(0, 100),
        value: role,
        description: `Iscriviti come ${ROLE_LABELS[role] || role}`.slice(0, 100)
      })))
  );
}

function buildPlayerSignupModal(role) {
  const modal = new ModalBuilder()
    .setCustomId(`player_signup_modal_${role}`)
    .setTitle(`Iscrizione Player • ${role}`);
...
async function getBotSetting(key, defaultValue = null) {
  const { data } = await supabase
    .from('bot_settings')
    .select('value')
    .eq('key', key)
...
async function findRoleForGameRole(guild, gameRole) {
  const envKey = `ROLE_${gameRole}_ID`;
  const configured = process.env[envKey];
  if (configured) {
    const roleById = await guild.roles.fetch(configured).catch(() => null);
    if (roleById) return roleById;
  }

  await guild.roles.fetch().catch(() => null);
  const label = ROLE_LABELS[gameRole];
  return guild.roles.cache.find(role =>
    role.name.toUpperCase() === gameRole ||
    role.name.toLowerCase() === String(label || '').toLowerCase()
  ) || null;
}

async function assignPlayerRegistrationRoles(member, gameRole) {
  await member.roles.add(PLAYER_ROLE_ID).catch(() => null);
  const role = await findRoleForGameRole(member.guild, gameRole);
  if (role) await member.roles.add(role.id).catch(() => null);
}

async function savePlayerRegistration(interaction, payload) {
  const { data, error } = await supabase
...
function buildRegistrationCompleteEmbed(registration) {
  return new EmbedBuilder()
    .setTitle('✅ Iscrizione player completata')
    .setColor(0x2ecc71)
    .addFields(
      { name: 'Player', value: `<@${registration.discord_id}>`, inline: true },
      { name: 'Nome', value: registration.name || 'N/D', inline: true },
      { name: 'Ruolo', value: `${registration.primary_role} • ${ROLE_LABELS[registration.primary_role] || registration.primary_role}`, inline: true },
      { name: 'Console', value: registration.platform || 'N/D', inline: true },
      { name: 'ID Console', value: registration.platform_id || 'N/D', inline: true },
      { name: 'Overall', value: String(registration.overall || 'N/D'), inline: true }
    )
    .setFooter({ text: 'RPCI • Iscrizione Player' })
    .setTimestamp();
}

function distributePlayersByRole(players, teamNames, maxPerTeam = null) {
  const teams = teamNames.map((name, index) => ({
    name,
    seed: index + 1,
    players: [],
    roleCounts: Object.fromEntries(GAME_ROLES.map(role => [role, 0]))
  }));

  const grouped = Object.fromEntries(GAME_ROLES.map(role => [role, []]));
  for (const player of players) {
    const role = GAME_ROLES.includes(player.primary_role) ? player.primary_role : 'ATT';
    grouped[role].push(player);
  }

  for (const role of GAME_ROLES) {
    const rolePlayers = shuffleArray(grouped[role] || []);
    for (const player of rolePlayers) {
      const availableTeams = teams.filter(team => !maxPerTeam || team.players.length < maxPerTeam);
      const pool = availableTeams.length ? availableTeams : teams;
      pool.sort((a, b) => {
        const roleDiff = (a.roleCounts[role] || 0) - (b.roleCounts[role] || 0);
...
function buildDraftResultEmbed(teams) {
  const embed = new EmbedBuilder()
    ...
    .setColor(0xd4af37)
    .setDescription('Le squadre sono state generate bilanciando i player per ruolo principale.')
    .setFooter({ text: 'RPCI • Player Draft System' })
    .setTimestamp();

  for (const team of teams.slice(0, 25)) {
    const roleSummary = GAME_ROLES
      .filter(role => (team.roleCounts[role] || 0) > 0)
      .map(role => `${role}: ${team.roleCounts[role]}`)
      .join(' • ') || 'N/D';
    embed.addFields({
      name: `🏟️ ${team.name} • ${team.players.length} player`,
      value: roleSummary.slice(0, 1024),
      inline: false
    });
  }
...
async function saveDraftTeams(teams, createdByDiscordId) {
  await supabase.from('draft_assignments').delete().neq('id', '00000000-0000-0000-0000-000000000000');
  await supabase.from('draft_teams').delete().neq('id', '00000000-0000-0000-0000-000000000000');
  ...
  for (const team of teams) {
    ...
    if (rows.length) {
      const { error: assignError } = await supabase.from('draft_assignments').insert(rows);
      if (assignError) throw assignError;
    }

    await supabase
      .from('player_registrations')
      .update({ status: 'assigned', assigned_team_name: team.name })
      .in('id', team.players.map(p => p.id));
  }
}

async function getDraftTeamByName(name) {
  const { data } = await supabase
    .from('draft_teams')
    .select('*')
    .ilike('name', name)
  ...
  try {
    if (interaction.isChatInputCommand()) {
      if (interaction.commandName === 'avvia_iscrizioni_player') {
        ...
        await setBotSetting('player_registrations_open', true);
        await interaction.channel.send(buildPlayerSignupPanel());
      ...
      if (interaction.commandName === 'chiudi_iscrizioni_player') {
        ...
        const member = await interaction.guild.members.fetch(interaction.user.id);
        if (!isStaffMember(member)) {
          return interaction.reply({ content: '❌ Solo lo staff può chiudere le iscrizioni player.', flags: MessageFlags.Ephemeral });
        }
        await setBotSetting('player_registrations_open', false);
      ...
      if (interaction.commandName === 'sorteggia_squadre') {
        ...
        const teamCount = interaction.options.getInteger('numero_squadre');
        const maxPerTeam = interaction.options.getInteger('max_player');
        const namesRaw = interaction.options.getString('nomi_squadre');
        if (!Number.isInteger(teamCount) || teamCount < 2 || teamCount > 40) {
          return interaction.reply({ content: '❌ Numero squadre non valido. Usa un valore tra 2 e 40.', flags: MessageFlags.Ephemeral });
        }

        let teamNames = namesRaw
          ? namesRaw.split(',').map(x => x.trim()).filter(Boolean)
          : [];
        if (teamNames.length && teamNames.length !== teamCount) {
          return interaction.reply({ content: `❌ Hai indicato ${teamNames.length} nomi, ma il numero squadre è ${teamCount}.`, flags: MessageFlags.Ephemeral });
        }
        if (!teamNames.length) teamNames = Array.from({ length: teamCount }, (_, i) => `Squadra ${i + 1}`);
        ...
        if (!players || players.length < teamCount) {
          return interaction.reply({ content: '❌ Non ci sono abbastanza player iscritti per sorteggiare queste squadre.', flags: MessageFlags.Ephemeral });
        }

        const teams = distributePlayersByRole(players, teamNames, maxPerTeam || null);
        await saveDraftTeams(teams, interaction.user.id);

        return interaction.reply({
          content: '✅ Sorteggio completato. I dati sono stati salvati nel database.',
      ...
      if (interaction.commandName === 'staff_assegna_capitano') {
        ...
        const team = await getDraftTeamByName(teamName);
        if (!team) return interaction.reply({ content: '❌ Squadra sorteggiata non trovata.', flags: MessageFlags.Ephemeral });
        ...
        await supabase
          .from('draft_teams')
          .update({ captain_discord_id: targetUser.id, captain_discord_tag: targetUser.tag, updated_at: new Date().toISOString() })
          .eq('id', team.id);

        return interaction.reply({ content: `✅ <@${targetUser.id}> è ora capitano di **${team.name}**.`, flags: MessageFlags.Ephemeral });
      }
      ...
      if (interaction.commandName === 'staff_rimuovi_capitano') {
        ...
        await supabase.from('draft_teams').update({ captain_discord_id: null, captain_discord_tag: null }).eq('captain_discord_id', targetUser.id);
        return interaction.reply({ content: `✅ Ruolo capitano rimosso a <@${targetUser.id}>.`, flags: MessageFlags.Ephemeral });
      }
    }

    if (interaction.isButton() && interaction.customId === 'player_signup_start') {
      const open = await getBotSetting('player_registrations_open', false);
      if (!open) {
        return interaction.reply({ content: '❌ Le iscrizioni player sono attualmente chiuse.', flags: MessageFlags.Ephemeral });
      }
      return interaction.reply({
        content: 'Seleziona il tuo **ruolo principale**.',
        components: [buildPlayerSignupRoleSelect()],
        flags: MessageFlags.Ephemeral
      });
    }

    if (interaction.isStringSelectMenu() && interaction.customId === 'player_signup_role_select') {
      const role = interaction.values[0];
      if (!GAME_ROLES.includes(role)) {
        return interaction.reply({ content: '❌ Ruolo non valido.', flags: MessageFlags.Ephemeral });
      }
      playerSignupDrafts.set(interaction.user.id, { primaryRole: role });
      return interaction.showModal(buildPlayerSignupModal(role));
    }

    if (interaction.isModalSubmit() && interaction.customId.startsWith('player_signup_modal_')) {
      const role = interaction.customId.replace('player_signup_modal_', '');
      if (!GAME_ROLES.includes(role)) {
        return interaction.reply({ content: '❌ Ruolo non valido.', flags: MessageFlags.Ephemeral });
      }
      ...
      const registration = await savePlayerRegistration(interaction, {
        name,
        age,
        platform,
        platformId,
      ...
      const member = await interaction.guild.members.fetch(interaction.user.id).catch(() => null);
      if (member) await assignPlayerRegistrationRoles(member, role);

      return interaction.reply({
        content: '✅ Iscrizione completata. Ti sono stati assegnati i ruoli Discord disponibili.',
        embeds: [buildRegistrationCompleteEmbed(registration)],
        flags: MessageFlags.Ephemeral
      });
    }
  } catch (error) {
    console.error('Errore Player Draft System:', error);
    if (interaction.deferred || interaction.replied) {
      return interaction.editReply('❌ Errore sistema player/draft: ' + error.message);
    }
    return interaction.reply({ content: '❌ Errore sistema player/draft: ' + error.message, flags: MessageFlags.Ephemeral });
  }
});
