...
async function logBotInteraction(interaction) {
  try {
    if (!interaction.guild) return;
    const channel = await client.channels.fetch(BOT_LOG_CHANNEL_ID).catch(() => null);
    if (!channel) return;

    let tipo = 'Interazione';
    let nome = 'N/D';
    let dettagli = 'Nessun dettaglio';

    if (interaction.isChatInputCommand()) {
      tipo = 'Comando slash';
      nome = `/${interaction.commandName}`;
      dettagli = interaction.options?.data?.map(option => `• ${option.name}: ${option.value ?? 'N/D'}`).join('\n') || 'Nessuna opzione';
    } else if (interaction.isButton()) {
      tipo = 'Pulsante';
      nome = interaction.customId || 'N/D';
      dettagli = `Messaggio: ${interaction.message?.id || 'N/D'}`;
    } else if (interaction.isStringSelectMenu()) {
      tipo = 'Menù a tendina';
      nome = interaction.customId || 'N/D';
      dettagli = `Valori: ${(interaction.values || []).join(', ') || 'N/D'}`;
    } else if (interaction.isModalSubmit()) {
      tipo = 'Modal compilato';
      nome = interaction.customId || 'N/D';
      dettagli = 'Modulo inviato dall’utente.';
    }

    await channel.send({
      embeds: [
        new EmbedBuilder()
          .setTitle('🧾 Log interazione bot')
          .setColor(0x5865f2)
          .addFields(
            { name: 'Tipo', value: tipo, inline: true },
            { name: 'Nome / ID', value: String(nome).slice(0, 1024), inline: true },
            { name: 'Utente', value: `${interaction.user.tag} (<@${interaction.user.id}>)`, inline: false },
            { name: 'Canale', value: interaction.channelId ? `<#${interaction.channelId}>` : 'DM / N/D', inline: true },
            { name: 'Dettagli', value: String(dettagli).slice(0, 1024), inline: false }
          )
          .setFooter({ text: 'RPCI • Log Staff' })
          .setTimestamp()
      ]
    }).catch(() => null);
  } catch (error) {
    console.error('Errore log interazione:', error);
  }
}
...
client.on('interactionCreate', async interaction => {
  try {
    await logBotInteraction(interaction).catch(() => null);
    if (interaction.isChatInputCommand()) {
