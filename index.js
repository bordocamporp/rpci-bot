...
function getTierFromOverall(overall) {
  ...
  if (value <= 15) return 'ROOKIE';
  if (value <= 40) return 'BASSA';
  if (value <= 90) return 'MEDIA';
  if (value <= 170) return 'ALTA';
  if (value <= 300) return 'TOP';
  return 'LEGGENDA';
}
  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === 'stagione_terminata') {
        ...
        return interaction.editReply(`✅ Stagione terminata. Backup creato: **${backup.label}** • Contratti scalati: **${result.scaled}** • Free Agent creati: **${result.expired}**.`);
      }
