  ...
  if (
    ...
    await supabase
      ...
      .update({
        ...
        salary_per_year: getSalaryByTier(offer.player_tier || 'BASSA'),
response_status: 'accepted'
      })
...
async function upsertFreeAgentProfile(profile) {
  const payload = {
    ...
    player_tier: profile.player_tier || 'BASSA',
    performance_points: profile.performance_points || 0,
    salary_per_year: profile.salary_per_year || TIER_SALARIES.BASSA,
...
async function makePlayerFreeAgent(guild, appPlayer, reason = 'contract_expired') {
  ...
  const profile = await upsertFreeAgentProfile({
    ...
    player_tier: appPlayer.player_tier || 'BASSA',
    performance_points: appPlayer.performance_points || 0,
    salary_per_year: appPlayer.salary_per_year || TIER_SALARIES.BASSA,
  ...
  if (profile?.application_player_id) {
    await supabase
      ...
      .update({
        ...
        is_free_agent: false,
        contract_years: offer.contract_years,
        salary_per_year: offer.salary || getSalaryByTier(profile?.player_tier || 'BASSA'),
})
...
` +
    ...
    if (interaction.isButton()) {
      if (
        ...
        await supabase
          .from('club_application_players')
          .update({ response_status: player.responseStatus })
          .eq('application_id', draft.applicationId)
          .eq('discord_id', playerDiscordId);
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId === 'looking_description_modal') {
        ...
        const profile = await upsertFreeAgentProfile({
          ...draft,
          secondary_role: draft.secondary_role || 'NO',
          player_tier: currentClub?.row?.player_tier || 'BASSA',
          performance_points: currentClub?.row?.performance_points || 0,
          salary_per_year: currentClub?.row?.salary_per_year || TIER_SALARIES.BASSA,
