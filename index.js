...
(async () => {
  try {
    ...
    for (const rawCommand of commands.filter(Boolean)) {
      const commandJson = typeof rawCommand.toJSON === 'function' ? rawCommand.toJSON() : rawCommand;
    ...
    if (!prepared.length) {
      console.error('❌ Nessun comando preparato: controllo conversione SlashCommandBuilder fallito.');
    }
