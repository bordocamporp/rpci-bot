...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('competition_modal_')) {
      ...
      const logoRaw = interaction.fields.getTextInputValue('logo_url')?.trim() || '';
      const nation = interaction.fields.getTextInputValue('nation')?.trim() || '';
      const settingsRaw = interaction.fields.getTextInputValue('settings')?.trim() || '';
      ...
      if (clubs.length < 2) {
        return interaction.reply({ content: '❌ Servono almeno 2 club approvati per creare una competizione.', flags: MessageFlags.Ephemeral });
      }
      ...
      const draft = {
        ...
        name,
        season,
        logoUrl: logoRaw && logoRaw.toUpperCase() !== 'NO' ? logoRaw : null,
        nation: nation || null,
        settings: parseCompetitionSettings(type, settingsRaw),
        clubs,
        clubPage: 0,
        selectedClubIds: []
      };
