...
function buildClubSearchPanel() {
  const embed = new EmbedBuilder()
    .setColor(0xf1c40f)
    .setTitle('🏆 IL TUO CLUB CERCA GIOCATORI?')
    .setDescription(
      'Pubblica il tuo annuncio ufficiale RPCI e trova nuovi player ⚽\n\n' +
      '📋 Potrai inserire:\n' +
      '• nome club\n' +
      '• piattaforma\n' +
      '• ruoli cercati\n' +
      ...
      '📩 Premi il pulsante qui sotto per creare l’annuncio del tuo club.'
    )
    .setFooter({ text: 'RPCI • Club cerca player' })
    .setTimestamp();
...
` +
    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId === 'club_search_create') {
        ...
        const club = new TextInputBuilder()
        ...
        const roles = new TextInputBuilder()
          ...
          .setPlaceholder('POR, DC, ATT...')
          .setStyle(TextInputStyle.Short)
          .setRequired(true);

        const goals = new TextInputBuilder()
          .setCustomId('club_goals')
          .setLabel('OBIETTIVI / DESCRIZIONE')
          .setStyle(TextInputStyle.Paragraph)
          .setRequired(true);
        ...
        modal.addComponents(
          new ActionRowBuilder().addComponents(club),
  ...
  status: 'pending'
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId === 'club_search_modal') {
        ...
        const platform = interaction.fields.getTextInputValue('club_platform');
        const roles = interaction.fields.getTextInputValue('club_roles');
        const goals = interaction.fields.getTextInputValue('club_goals');
        const times = interaction.fields.getTextInputValue('club_times');
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: '⚽ Ruoli cercati', value: roles, inline: false },
            { name: '📋 Obiettivi', value: goals.slice(0,1024), inline: false },
            { name: '🕒 Disponibilità', value: times, inline: false }
          )
          .setFooter({ text: 'RPCI • Club cerca player' })
          .setTimestamp();
