...
function buildPanel() {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      '• Registrare ufficialmente il proprio club\n' +
      '• Inserire il nome squadra\n' +
      '• Caricare il logo ufficiale del club\n' +
      '• Inviare la richiesta allo staff RPCI\n\n' +

      '⚠️ **Nota importante:**\n' +
      'In questa fase viene registrato solo il **club**.\n' +
      'L’iscrizione dei giocatori verrà fatta successivamente con un comando dedicato.\n\n' +

      'Premi il pulsante qui sotto per iniziare l’iscrizione del club.'
    )
    .setFooter({
        ...
        if (accepted) {
          ...
          if (captainUser) {
            await captainUser.send(
              `✅ Il tuo club **${app.clubs?.name || 'RPCI'}** è stato approvato dallo staff RPCI.\n\nOra potrai procedere con l’iscrizione dei giocatori quando il sistema rosa sarà attivo.`
            ).catch(() => null);
          }
        }
