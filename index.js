...
function sanitizeRegisteredCommands(command) {
  const clone = JSON.parse(JSON.stringify(command));
  function cleanOptions(options, path) {
    if (!Array.isArray(options)) return [];
    return options.filter(option => {
      if (!option || typeof option !== 'object') return false;
      if (!option.type) {
        console.warn(`⚠️ Opzione slash rimossa perché senza type: ${path}.${option?.name || 'unknown'}`);
        return false;
      }
      return true;
    }).map(option => {
      if (Array.isArray(option.options)) option.options = cleanOptions(option.options, `${path}.${option.name}`);
      if (Array.isArray(option.choices)) {
        option.choices = option.choices.filter(choice => choice && typeof choice.name === 'string' && Object.prototype.hasOwnProperty.call(choice, 'value') && choice.value !== undefined && choice.value !== null);
      }
      return option;
    });
  }
  clone.options = cleanOptions(clone.options || [], clone.name);
  return clone;
}

function findBadSlashOptions(command) {
  const bad = [];
  function scan(options, path) {
    for (const option of options || []) {
      if (!option.type) bad.push(`${path}.${option.name || 'unknown'}`);
      if (option.options) scan(option.options, `${path}.${option.name || 'unknown'}`);
    }
  }
  scan(command.options || [], command.name);
  return bad;
}
...
(async () => {
  try {
    ...
    const safeCommands = commands.map((command, index) => {
      const bad = findBadSlashOptions(command);
      if (bad.length) console.warn(`⚠️ Comando ${index} (${command.name}) aveva opzioni non valide: ${bad.join(', ')}`);
      return sanitizeRegisteredCommands(command);
    });
    console.log('Comandi registrati:', safeCommands.map(c => c.name).join(', '));
    await rest.put(
