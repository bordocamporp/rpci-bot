...
function buildCompetitionComponents(draft) {
  ...
  if (draft.type === 'league') {
    ...
    components.push(buildNumberSelect('competition_promoted_select', 'Promosse: 0-10', 0, 10, draft.settings?.promosse ?? 0));
    components.push(buildNumberSelect('competition_relegated_select', 'Retrocesse: 0-10', 0, 10, draft.settings?.retrocesse ?? 0));
  }
...
` +
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId.startsWith('competition_modal_')) {
        ...
        const logoInput = interaction.fields.getTextInputValue('logo_url')?.trim();
        ...
        competitionDrafts.set(interaction.user.id, {
          ...
          name,
          season,
          logoUrl: logoInput && logoInput.toUpperCase() !== 'NO' ? logoInput : null,
          nation,
        ...
        return interaction.reply({
          content: '✅ Dati competizione salvati. Ora seleziona le squadre partecipanti.',
          embeds: [buildCompetitionEmbed(draft)],
          components: buildCompetitionComponents(draft),
          flags: MessageFlags.Ephemeral
        });
