...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName('asta_live')
    .setDescription('Crea un annuncio asta live RPCI')
    .addStringOption(option =>
      option
        .setName('giocatore')
        .setDescription('Nome del giocatore in asta')
        .setRequired(true)
    )
    .addIntegerOption(option =>
      option
        .setName('base')
        .setDescription('Prezzo base asta in crediti')
        .setRequired(true)
    )
    .addStringOption(option =>
      option
        .setName('scadenza')
        .setDescription('Scadenza asta, esempio: oggi 23:00')
        .setRequired(true)
    )
    .addStringOption(option =>
      option
        .setName('note')
        .setDescription('Note aggiuntive opzionali')
        .setRequired(false)
    ),
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === 'asta_live') {
        ...
        if (!member?.roles.cache.has(CAPTAIN_ROLE_ID) && !isStaffMember(member)) {
          return interaction.reply({
            content: '❌ Solo capitani e staff possono creare aste live.',
            flags: MessageFlags.Ephemeral
          });
        }

        const giocatore = interaction.options.getString('giocatore');
        ...
        if (!Number.isInteger(base) || base < 1) {
          return interaction.reply({
            content: '❌ Prezzo base non valido.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        const embed = new EmbedBuilder()
          .setColor(0xd4af37)
          .setTitle('🔨 ASTA LIVE RPCI')
          .setDescription(`È stata aperta una nuova asta live ufficiale RPCI.`)
          .addFields(
            { name: '👤 Giocatore', value: giocatore, inline: true },
            { name: '💰 Base asta', value: `${base} crediti`, inline: true },
            { name: '⏰ Scadenza', value: scadenza, inline: true },
            { name: '👑 Creatore', value: `<@${interaction.user.id}>`, inline: true },
            { name: '📝 Note', value: note.slice(0, 1024), inline: false }
          )
          .setFooter({ text: 'RPCI • Aste live ufficiali' })
          .setTimestamp();

        return interaction.reply({
          content: '🔨 **NUOVA ASTA LIVE RPCI**',
          embeds: [embed]
        });
      }
