...
function getTeamSide(match, applicationId, clubId = null) {
  if (match.home_application_id && match.home_application_id === applicationId) return 'home';
  if (match.away_application_id && match.away_application_id === applicationId) return 'away';

  if (clubId && match.home_club_id === clubId) return 'home';
  if (clubId && match.away_club_id === clubId) return 'away';

  return null;
}

function getScoreForTeam(match, report) {
  const side = getTeamSide(match, report.application_id, report.club_id || null);
  if (side === 'home') {
...
async function getCaptainOpenMatches(captainDiscordId) {
  ...
  if (!clubId) return { captainApplication: app, matches: [] };
  ...
  if (error) {
    console.error('Errore getCaptainOpenMatches:', error);
    return { captainApplication: app, matches: [] };
  }

  return { captainApplication: app, matches: data || [] };
}

function buildMatchSelect(matches, applicationId, clubId = null) {
  return new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .addOptions(
        matches.slice(0, 25).map(match => {
          const isHome = (match.home_application_id && match.home_application_id === applicationId) || (clubId && match.home_club_id === clubId);
          ...
          const label = `${yourClub || 'Tua squadra'} vs ${oppClub || 'Avversario'}`.slice(0, 100);
          return {
            label,
            description: match.matchday ? `Giornata: ${match.matchday}` : 'Partita in calendario',
            value: match.id
          };
        })
      )
...
async function submitMatchReport(interaction, draft) {
  const side = getTeamSide(draft.match, draft.applicationId, draft.clubId);
...
async function saveCompetitionDraft(draft, createdByDiscordId) {
  ...
  if (selected.length > 0) {
    const rows = selected.map((club, index) => ({
      ...
      club_id: club.id,
      club_name: club.name,
      application_id: club.application_id,
      group_name: null,
      seed_number: index + 1
    }));
...
async function getApprovedClubsWithApplications() {
  ...
  for (const app of data) {
    ...
    const clubId = club?.id || app.club_id;
...
async function getExistingLeagueCompetitionsForLink(currentName = null) {
  const { data, error } = await supabase
    .from('competitions')
    .select('id, name, season, type, status')
    .eq('type', 'league')
    .order('created_at', { ascending: false });
  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === 'referto') {
        ...
        matchReportDrafts.set(interaction.user.id, {
          applicationId: captainApplication.id,
          clubId: captainApplication.club_id || captainApplication.clubs?.id,
        ...
        return interaction.reply({
          ...
          components: [buildMatchSelect(matches, captainApplication.id, captainApplication.club_id || captainApplication.clubs?.id)],
          flags: MessageFlags.Ephemeral
        });
      }
    ...
    if (interaction.isStringSelectMenu()) {
      ...
      if (interaction.customId === 'match_report_match_select') {
        ...
        const isHomeForDraft = getTeamSide(match, draft.applicationId, draft.clubId) === 'home';
        const rosterClubId = isHomeForDraft ? match.home_club_id : match.away_club_id;
        const roster = rosterClubId ? await getClubRoster(rosterClubId) : [];
