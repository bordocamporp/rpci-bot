...
` +
  ...
  contract_years: Number(draft.contractYears),
  transfer_fee: Number(draft.transferFee || 0),
  ...
  status: 'pending'
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId === 'offer_amounts_modal_submit') {
        ...
        if (!Number.isFinite(transferFee) || transferFee < 0 || !Number.isInteger(transferFee)) {
          return interaction.reply({ content: '❌ Prezzo acquisto non valido.', flags: MessageFlags.Ephemeral });
        }

        if (!Number.isFinite(salary) || salary < 0 || !Number.isInteger(salary)) {
          return interaction.reply({ content: '❌ Stipendio non valido.', flags: MessageFlags.Ephemeral });
        }
