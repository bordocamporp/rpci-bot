...
const liveAuctionDrafts = new Map();
...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .setDescription('Staff: pubblica il pannello Club cerca player'),

  new SlashCommandBuilder()
    .setName('pannello_club_cerca_player')
    .setDescription('Staff: pubblica il pannello Club cerca player'),

  new SlashCommandBuilder()
    .setName('pannello_cerco_squadra')
    .setDescription('Staff: pubblica il pannello player cerca squadra'),
...
async function getCaptainApprovedClub(captainDiscordId) {
  const { data, error } = await supabase
    .from('club_applications')
    .select('id, club_id, captain_discord_id, status, clubs(id, name, logo_url, short_name)')
  ...
  if (error) {
    console.error('Errore getCaptainApprovedClub:', error);
    return null;
  }
...
function buildClubSearchPanel() {
  const embed = new EmbedBuilder()
    .setColor(0xd4af37)
    .setTitle('🏆 CLUB CERCA PLAYER')
    .setDescription(
      'Il tuo club sta cercando nuovi giocatori per la rosa RPCI?\n\n' +
      'Premi il pulsante qui sotto e pubblica un annuncio ufficiale con:\n\n' +
      '🎮 **Piattaforma**\n' +
      '⚽ **Ruoli cercati**\n' +
      '🎯 **Obiettivi del club**\n' +
      '🕒 **Orari e disponibilità richieste**\n\n' +
      '⚠️ Solo i capitani con club approvato possono creare annunci.'
    )
    .setFooter({ text: 'RPCI • Scouting ufficiale club' })
    .setTimestamp();

  const button = new ButtonBuilder()
    .setCustomId('club_search_create')
    .setLabel('CREA ANNUNCIO CLUB')
    .setEmoji('🏆')
...
function buildPlayerSearchPanel() {
  const embed = new EmbedBuilder()
    .setColor(0x5865f2)
    .setTitle('🔎 CERCHI UNA SQUADRA PRO CLUB?')
    .setDescription(
      'Pubblica il tuo profilo giocatore RPCI e fatti trovare dai capitani ⚽\n\n' +
      '📋 **Cosa potrai inserire:**\n' +
      '• Nome player\n' +
      '• Età\n' +
      '• Ruolo\n' +
      '• Piattaforma\n' +
      ...
      '• Club precedenti\n\n' +
      '📩 Premi il pulsante qui sotto per creare il tuo annuncio ufficiale.'
    )
    .setFooter({ text: 'RPCI • Player cerca squadra' })
    .setTimestamp();

  const button = new ButtonBuilder()
    .setCustomId('player_search_create')
    .setLabel('CREA ANNUNCIO PLAYER')
    .setEmoji('🔎')
...
async function finalizeApplication(interaction, draft) {
  ...
  const playersToInsert = draft.players.map(player => ({
    ...
    salary_per_year: TIER_SALARIES.BASSA,
    contract_total_value: TIER_SALARIES.BASSA * Number(player.contract_years || 1),
  ...
  if (
    ...
    await supabase
      ...
      .update({
        ...
        contract_total_value: getSalaryByTier(offer.player_tier || 'BASSA') * Number(offer.contract_years || 1),
        response_status: 'accepted'
      })
  ...
  if (profile?.application_player_id) {
    await supabase
      ...
      .update({
        ...
        contract_total_value: Number(offer.salary || getSalaryByTier(profile?.player_tier || 'BASSA')) * Number(offer.contract_years || 1)
      })
...
async function getApprovedClubsForAuction() {
  const { data, error } = await supabase
    .from('club_applications')
    .select('id, club_id, captain_discord_id, status, clubs(id, name, logo_url, short_name)')
    .eq('status', 'approved')
  ...
  if (error || !data) {
    console.error('Errore getApprovedClubsForAuction:', error);
    return [];
  }
  ...
  for (const app of data) {
    const clubId = app.club_id || app.clubs?.id;
    ...
    clubs.push({
      ...
      clubId,
      captainDiscordId: app.captain_discord_id,
      name: app.clubs?.name || 'Club RPCI',
      logoUrl: app.clubs?.logo_url || null,
      shortName: app.clubs?.short_name || null
    });
  }

  return clubs.sort((a, b) => String(a.name).localeCompare(String(b.name)));
}
  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === 'asta_live') {
        ...
        const clubs = await getApprovedClubsForAuction();
        if (!clubs.length) {
          return interaction.reply({
            content: '❌ Nessun club approvato disponibile.',
            flags: MessageFlags.Ephemeral
          });
        }

        const draftId = `${interaction.user.id}_${Date.now()}`;
        liveAuctionDrafts.set(interaction.user.id, {
          id: draftId,
      ...
      if (interaction.commandName === 'club_cerca_squadra') {
        const member = await interaction.guild.members.fetch(interaction.user.id).catch(() => null);

        if (!isStaffMember(member)) {
          return interaction.reply({
            content: '❌ Solo lo staff può pubblicare il pannello Club cerca player.',
            flags: MessageFlags.Ephemeral
          });
        }

        return interaction.reply(buildClubSearchPanel());
      }

      if (interaction.commandName === 'pannello_club_cerca_player') {
        const member = await interaction.guild.members.fetch(interaction.user.id).catch(() => null);
...
if (interaction.commandName === 'pannello_cerco_squadra') {
        const member = await interaction.guild.members.fetch(interaction.user.id).catch(() => null);
        if (!isStaffMember(member)) {
          return interaction.reply({
            content: '❌ Solo lo staff può pubblicare il pannello cerca squadra.',
            flags: MessageFlags.Ephemeral
          });
        }

        return interaction.reply(buildPlayerSearchPanel());
      }
    ...
    if (interaction.isStringSelectMenu()) {
      ...
      if (interaction.customId === 'auction_club_select') {
        const draft = liveAuctionDrafts.get(interaction.user.id);
        if (!draft) {
          return interaction.reply({
            content: '❌ Nessuna asta live in corso.',
            flags: MessageFlags.Ephemeral
          });
        }

        const club = draft.clubs.find(c => c.applicationId === interaction.values[0]);
        if (!club) {
          return interaction.reply({
            content: '❌ Club non trovato.',
            flags: MessageFlags.Ephemeral
          });
        }

        const players = await getAuctionClubPlayers(club.applicationId);
        if (!players.length) {
          return interaction.update({
            content: '❌ Questo club non ha giocatori accettati disponibili.',
            components: []
          });
        }
        ...
        return interaction.update({
          content: `🏟️ Club selezionato: **${club.name}**\nOra seleziona il giocatore.`,
          components: [buildAuctionPlayerSelect(players)]
        });
      }

      if (interaction.customId === 'auction_player_select') {
        const draft = liveAuctionDrafts.get(interaction.user.id);
        if (!draft || !draft.selectedClub) {
          return interaction.reply({
            content: '❌ Nessuna asta live in corso.',
            flags: MessageFlags.Ephemeral
          });
        ...
        const player = draft.players.find(p => p.id === interaction.values[0]);
        if (!player) {
          return interaction.reply({
        ...
        const modal = new ModalBuilder()
          .setCustomId('auction_offer_modal')
          .setTitle('Offerta asta live');

        const offer = new TextInputBuilder()
          .setCustomId('offer_credits')
          .setLabel('OFFERTA / BASE ASTA IN CREDITI')
          .setPlaceholder('Esempio: 10')
          .setStyle(TextInputStyle.Short)
          .setRequired(true);

        modal.addComponents(new ActionRowBuilder().addComponents(offer));
        return interaction.showModal(modal);
      }
...
` +
    ...
    if (interaction.isButton()) {
      ...
      if (
        interaction.customId.startsWith('auction_captain_accept_') ||
        interaction.customId.startsWith('auction_captain_reject_') ||
        interaction.customId.startsWith('auction_player_accept_') ||
        interaction.customId.startsWith('auction_player_reject_')
      ) {
        const isCaptainAction = interaction.customId.startsWith('auction_captain_');
        const accepted = interaction.customId.includes('_accept_');
        const draftId = interaction.customId.split('_').slice(3).join('_');
        const draft = liveAuctionDrafts.get(draftId);

        if (!draft) {
          return interaction.reply({
            content: '❌ Proposta asta non trovata o scaduta.',
            flags: MessageFlags.Ephemeral
          });
        }

        if (isCaptainAction && interaction.user.id !== draft.clubCaptainDiscordId) {
          return interaction.reply({
            content: '❌ Solo il capitano proprietario può rispondere.',
            flags: MessageFlags.Ephemeral
          });
        }

        if (!isCaptainAction && interaction.user.id !== draft.playerDiscordId) {
          return interaction.reply({
            content: '❌ Solo il giocatore interessato può rispondere.',
            flags: MessageFlags.Ephemeral
          });
        }

        if (isCaptainAction) {
          if (draft.captainStatus !== 'pending') {
            return interaction.reply({ content: '⚠️ Hai già risposto.', flags: MessageFlags.Ephemeral });
          }
          draft.captainStatus = accepted ? 'accepted' : 'rejected';
        } else {
          if (draft.playerStatus !== 'pending') {
            return interaction.reply({ content: '⚠️ Hai già risposto.', flags: MessageFlags.Ephemeral });
          }
          draft.playerStatus = accepted ? 'accepted' : 'rejected';
        }

        if (!accepted) {
          liveAuctionDrafts.delete(draft.id);
        } else {
          await publishAuctionIfApproved(draft);
          if (draft.captainStatus === 'accepted' && draft.playerStatus === 'accepted') {
            liveAuctionDrafts.delete(draft.id);
          }
        }
        ...
        return interaction.update({
          content: accepted
            ? '✅ Hai accettato la proposta asta live RPCI.'
            : '❌ Hai rifiutato la proposta asta live RPCI.',
      ...
      if (interaction.customId === 'club_search_create') {
        ...
        const app = await getCaptainApprovedClub(interaction.user.id);
        if (!app) {
          return interaction.reply({
            content: '❌ Devi avere un club approvato dallo staff per creare un annuncio.',
            flags: MessageFlags.Ephemeral
          });
        }

        const modal = new ModalBuilder()
          .setCustomId('club_search_modal')
          .setTitle('Crea annuncio club');

        const platform = new TextInputBuilder()
          .setCustomId('club_platform')
          .setLabel('PIATTAFORMA')
          .setPlaceholder('PS5 / XBOX / PC')
          .setStyle(TextInputStyle.Short)
          .setRequired(true);

        const roles = new TextInputBuilder()
          .setCustomId('club_roles')
          .setLabel('RUOLI CERCATI')
          .setPlaceholder('POR, DC, TD, CC, ATT...')
          .setStyle(TextInputStyle.Short)
          .setRequired(true);

        const goals = new TextInputBuilder()
          .setCustomId('club_goals')
          .setLabel('OBIETTIVI DEL CLUB')
          .setPlaceholder('Competitivo, progetto serio, torneo estivo RPCI...')
          .setStyle(TextInputStyle.Paragraph)
          .setRequired(true);

        const times = new TextInputBuilder()
          .setCustomId('club_times')
          .setLabel('ORARI / DISPONIBILITÀ')
          .setPlaceholder('Esempio: sera 21:30-23:30')
          .setStyle(TextInputStyle.Short)
          .setRequired(true);

        const requirements = new TextInputBuilder()
          .setCustomId('club_requirements')
          .setLabel('REQUISITI PLAYER')
          .setPlaceholder('Microfono, serietà, esperienza, presenza...')
          .setStyle(TextInputStyle.Paragraph)
          .setRequired(true);
        ...
        modal.addComponents(
          ...
          new ActionRowBuilder().addComponents(goals),
          new ActionRowBuilder().addComponents(times),
...
if (interaction.customId === 'player_search_create') {
        ...
        const age = new TextInputBuilder()
          .setCustomId('age')
          .setLabel('ETÀ')
          .setPlaceholder('Inserisci solo il numero')
          .setStyle(TextInputStyle.Short)
          .setRequired(true);
        ...
        const role = new TextInputBuilder()
          .setCustomId('role')
          .setLabel('RUOLO')
          .setPlaceholder('Esempio: POR, DC, CC, ATT')
          .setStyle(TextInputStyle.Short)
          .setRequired(true);
  ...
  status: 'pending'
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId === 'auction_offer_modal') {
        const draft = liveAuctionDrafts.get(interaction.user.id);
        if (!draft || !draft.selectedClub || !draft.selectedPlayer) {
          return interaction.reply({
            content: '❌ Nessuna asta live in corso.',
            flags: MessageFlags.Ephemeral
          });
        ...
        const offerCredits = Number(interaction.fields.getTextInputValue('offer_credits').trim());
        if (!Number.isInteger(offerCredits) || offerCredits < 1) {
          return interaction.reply({
            content: '❌ Offerta non valida. Inserisci un numero di crediti.',
            flags: MessageFlags.Ephemeral
          });
        }

        const playerDiscordId = draft.selectedPlayer.discord_id;
        if (!playerDiscordId) {
          return interaction.reply({
            content: '❌ Il giocatore selezionato non ha un ID Discord valido.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        const auctionDraft = {
          ...
          requesterDiscordId: interaction.user.id,
          channelId: interaction.channelId,
          clubApplicationId: draft.selectedClub.applicationId,
          clubId: draft.selectedClub.clubId,
          clubName: draft.selectedClub.name,
          clubLogoUrl: draft.selectedClub.logoUrl,
          clubCaptainDiscordId: draft.selectedClub.captainDiscordId,
          playerRowId: draft.selectedPlayer.id,
          playerDiscordId,
          playerConsoleId: draft.selectedPlayer.platform_id || 'N/D',
          playerPlatform: draft.selectedPlayer.platform || 'N/D',
          offerCredits,
          captainStatus: 'pending',
          playerStatus: 'pending'
        };

        liveAuctionDrafts.set(auctionDraft.id, auctionDraft);
        liveAuctionDrafts.delete(interaction.user.id);

        await sendAuctionApprovalRequests(auctionDraft);

        return interaction.reply({
          content:
            '✅ Proposta asta live inviata in privato al capitano del club e al giocatore.\n\n' +
            'L’asta verrà pubblicata solo se entrambi accettano.',
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (interaction.customId === 'club_search_modal') {
        const app = await getCaptainApprovedClub(interaction.user.id);
        if (!app) {
          return interaction.reply({
            content: '❌ Devi avere un club approvato dallo staff per pubblicare l’annuncio.',
            flags: MessageFlags.Ephemeral
          });
        }

        const clubName = app.clubs?.name || 'Club RPCI';
        const logoUrl = app.clubs?.logo_url || null;

        const platform = interaction.fields.getTextInputValue('club_platform').trim().toUpperCase();
        const roles = interaction.fields.getTextInputValue('club_roles').trim();
        const goals = interaction.fields.getTextInputValue('club_goals').trim();
        const times = interaction.fields.getTextInputValue('club_times').trim();
        const requirements = interaction.fields.getTextInputValue('club_requirements').trim();

        const embed = new EmbedBuilder()
          .setColor(0xd4af37)
          .setTitle('🏆 CLUB CERCA PLAYER')
          .setDescription(`Il club **${clubName}** è alla ricerca di nuovi giocatori per completare la rosa RPCI.`)
          .addFields(
            ...
            { name: '⚽ Ruoli cercati', value: roles.slice(0, 1024), inline: false },
            { name: '🎯 Obiettivi del club', value: goals.slice(0, 1024), inline: false },
            { name: '🕒 Orari / disponibilità', value: times.slice(0, 1024), inline: false },
            { name: '📌 Requisiti player', value: requirements.slice(0, 1024), inline: false }
          )
          .setFooter({ text: 'RPCI • Scouting ufficiale club' })
          .setTimestamp();
        ...
        return interaction.reply({
          content: '📢 **NUOVO ANNUNCIO CLUB RPCI**',
          embeds: [embed]
        });
      }

if (interaction.customId === 'player_search_modal') {
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: '📝 ID Console + Esperienze', value: details.slice(0, 1024), inline: false }
          )
          .setFooter({ text: 'RPCI • Player cerca squadra' })
          .setTimestamp();
...
function hasCommandChannelBypassRole(member) {
  return Boolean(member?.roles?.cache?.some(role => COMMAND_CHANNEL_ALLOWED_ROLE_IDS.includes(role.id)));
}
...
client.on('messageCreate', async message => {
  ...
  if (hasCommandChannelBypassRole(message.member)) return;
  ...
  if (protectedChannels.includes(message.channelId) && !hasCommandChannelBypassRole(message.member)) {
