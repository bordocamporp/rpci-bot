...
const CLUB_NEWS_CHANNEL_ID = '1507739357954572371';
const CONTRACT_DEPOSIT_CHANNEL_ID = '1508454065535979591';
const PLAYER_OFFICIAL_NEWS_CHANNEL_ID = '1507741237900214332';
...
const contractDepositDrafts = new Map();
...
const TIER_RANGES = [
  { tier: 'ROOKIE', min: 0, max: 20 },
  { tier: 'BRONZE', min: 21, max: 60 },
  { tier: 'SILVER', min: 61, max: 120 },
  { tier: 'GOLD', min: 121, max: 220 },
  { tier: 'ELITE', min: 221, max: Infinity }
];
...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName('deposito_contratti')
    .setDescription('Pubblica il pannello per depositare i contratti della rosa'),
...
function buildCaptainPanel() {
  const attachment = new AttachmentBuilder('./capitano-rpci.png', {
    name: 'capitano-rpci.png'
  });

  const embed = new EmbedBuilder()
    .setColor(0xd4af37)
    .setTitle('👑 DIVENTA CAPITANO RPCI')
    .setDescription(
      'Sei capitano di una squadra **PRO CLUB**?\n\n' +
      'Premi il pulsante qui sotto ed otterrai il ruolo **Capitano RPCI** e potrai:\n\n' +
      '• Registrare il tuo club\n' +
      '• Gestire trasferimenti\n' +
      '• Inviare referti match\n' +
      '• Partecipare alle competizioni ufficiali\n' +
      '• Fare il vero manager del tuo club comprando e vendendo giocatori\n\n' +
      '⚠️ **Un solo club per capitano.**'
    )
    .setImage('attachment://capitano-rpci.png')
    .setFooter({ text: 'RPCI • Sistema Capitani Ufficiale' })
    .setTimestamp();

  const button = new ButtonBuilder()
    .setCustomId('become_captain')
    .setLabel('DIVENTA CAPITANO')
    .setEmoji('👑')
...
function buildContractDepositPanel() {
  const embed = new EmbedBuilder()
    .setColor(0xd4af37)
    .setTitle('📩 DEPOSITO CONTRATTI RPCI')
    .setDescription(
      'Devi completare l’iscrizione della squadra depositando i contratti dei giocatori in rosa.\n\n' +
      '⚠️ **Importante prima di iniziare:**\n' +
      'Copia gli **ID Discord** dei giocatori che dovranno essere inseriti durante l’iscrizione.\n\n' +
      'Ogni giocatore dovrà avere:\n' +
      '• Nome giocatore\n' +
      '• ID Discord reale\n' +
      '• Console: PS5 / XBOX / PC\n' +
      '• ID console\n' +
      '• Anni contratto: da 1 a 3\n\n' +
      'Premi il pulsante qui sotto per iniziare.'
    )
    .setFooter({ text: 'RPCI • Deposito contratti ufficiale' })
    .setTimestamp();

  const button = new ButtonBuilder()
    .setCustomId('deposit_contracts_start')
    .setLabel('DEPOSITA CONTRATTO')
    .setEmoji('📩')
...
function buildContractPlayerCountSelect() {
  const options = Array.from({ length: 21 }, (_, i) => {
    const value = String(i + 5);
    return {
      label: `${value} giocatori`,
      value,
      description: `Deposita ${value} contratti`
    };
  });

  return new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      .setCustomId('contract_player_count_select')
      .setPlaceholder('Quanti giocatori devi iscrivere? Minimo 5')
      .setMinValues(1)
      .setMaxValues(1)
      .addOptions(options)
  );
...
function buildContractYearsSelect(index) {
  return new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      .setCustomId(`contract_years_select_${index}`)
      .setPlaceholder('ANNI CONTRATTO')
      .setMinValues(1)
      .setMaxValues(1)
      .addOptions([
...
function buildContractProgressEmbed(draft) {
  const current = draft.currentIndex + 1;
  const total = draft.totalPlayers;

  return new EmbedBuilder()
    .setColor(0xd4af37)
    .setTitle('📩 Deposito contratti rosa')
    .setDescription(
      `Stai inserendo il giocatore **${current}/${total}**.\n\n` +
      'Compila tutti i dati obbligatori. Il bot verificherà che l’ID Discord inserito esista realmente nel server.'
    )
    .addFields(
      { name: 'Club', value: draft.clubName || 'Club RPCI', inline: true },
      { name: 'Giocatori inseriti', value: `${draft.players.length}/${total}`, inline: true }
    )
    .setFooter({ text: 'RPCI • Deposito contratti' })
    .setTimestamp();
}

function buildOpenPlayerContractModalButton(index) {
  const button = new ButtonBuilder()
    .setCustomId(`contract_player_modal_${index}`)
    .setLabel(`INSERISCI DATI GIOCATORE ${index + 1}`)
    .setEmoji('📝')
    .setStyle(ButtonStyle.Primary);

  const cancel = new ButtonBuilder()
    .setCustomId('contract_deposit_cancel')
    .setLabel('ANNULLA')
    .setStyle(ButtonStyle.Danger);

  return new ActionRowBuilder().addComponents(button, cancel);
}

function buildContractConfirmRow() {
  const confirm = new ButtonBuilder()
    .setCustomId('contract_deposit_confirm')
    .setLabel('INVIA CONTRATTI')
    .setEmoji('✅')
    .setStyle(ButtonStyle.Success);
...
async function getCaptainClubForContracts(captainDiscordId) {
  const { data, error } = await supabase
    .from('club_applications')
    .select('id, club_id, captain_discord_id, status, clubs(id, name, logo_url)')
    .eq('captain_discord_id', captainDiscordId)
    .eq('status', 'approved')
  ...
  if (error) {
    console.error('Errore getCaptainClubForContracts:', error);
    return null;
  }
...
function buildContractSummaryEmbed(draft) {
  const lines = draft.players.map(player => {
    const icon =
      player.responseStatus === 'accepted'
        ? '✅'
        : player.responseStatus === 'rejected'
          ? '❌'
          : '⌛';

    return `${icon} **${player.playerName}** • <@${player.discordId}> • ${player.platform} • Contratto ${player.contractYears} anno/i`;
  });

  return new EmbedBuilder()
    .setColor(0xd4af37)
    .setTitle('📩 Deposito contratti in attesa conferme')
    .setDescription(
      `Il capitano <@${draft.captainDiscordId}> ha depositato i contratti per **${draft.clubName}**.\n\n` +
      `I giocatori devono accettare o rifiutare il deposito in privato.\n\n` +
      lines.join('\n')
    )
    .addFields(
      { name: 'Club', value: draft.clubName, inline: true },
      { name: 'Giocatori', value: `${draft.players.length}`, inline: true },
      { name: 'Scadenza', value: '24 ore dalla richiesta', inline: true }
    )
    .setFooter({ text: 'RPCI • Stato conferme contratti' })
    .setTimestamp();
}


async function sendPlayerOfficialAnnouncement(draft, player) {
  const channel = await client.channels.fetch(PLAYER_OFFICIAL_NEWS_CHANNEL_ID).catch(() => null);
  if (!channel) return;

  const embed = new EmbedBuilder()
    .setColor(0xd4af37)
    .setTitle('📝 UFFICIALE RPCI')
    .setDescription(
      `**${player.playerName}** è ufficialmente un nuovo giocatore di **${draft.clubName}**.`
    )
    .addFields(
      { name: '👤 Giocatore', value: player.playerName, inline: true },
      { name: '🏷️ Tag Discord', value: `<@${player.discordId}>`, inline: true },
      { name: '🏟️ Club', value: draft.clubName, inline: true },
      { name: '🎮 Console', value: player.platform, inline: true },
      { name: '🆔 ID Console', value: player.consoleId, inline: true },
      { name: '📄 Contratto', value: `${player.contractYears} anno/i`, inline: true }
    )
    .setFooter({ text: 'RPCI • Comunicati ufficiali mercato' })
    .setTimestamp();

  if (draft.clubLogoUrl) embed.setThumbnail(draft.clubLogoUrl);

  await channel.send({
    content: '✅ **COMUNICATO UFFICIALE RPCI**',
    embeds: [embed]
  }).catch(() => null);
}

async function updateContractSummaryMessage(draft) {
  if (!draft.summaryMessageId) return;

  const channel = await client.channels.fetch(CONTRACT_DEPOSIT_CHANNEL_ID).catch(() => null);
  if (!channel) return;

  const message = await channel.messages.fetch(draft.summaryMessageId).catch(() => null);
  if (!message) return;

  await message.edit({
    embeds: [buildContractSummaryEmbed(draft)]
  }).catch(() => null);
}

async function scheduleContractAutoReject(captainDiscordId, playerDiscordId) {
  setTimeout(async () => {
    const draft = contractDepositDrafts.get(captainDiscordId);
    if (!draft) return;

    const player = draft.players.find(p => p.discordId === playerDiscordId);
    if (!player || player.responseStatus !== 'pending') return;
    ...
    await updateContractSummaryMessage(draft);

    const user = await client.users.fetch(playerDiscordId).catch(() => null);
    if (user) {
      await user.send(
        `⏰ La richiesta di deposito contratto per **${draft.clubName}** è stata rifiutata automaticamente perché non hai risposto entro 24 ore.`
      ).catch(() => null);
    }
  }, 24 * 60 * 60 * 1000);
}

async function sendContractDecisionDM(draft, player) {
  const user = await client.users.fetch(player.discordId).catch(() => null);
  if (!user) return;

  const accept = new ButtonBuilder()
    .setCustomId(`contract_accept_${draft.captainDiscordId}_${player.discordId}`)
    .setLabel('ACCETTA')
    .setStyle(ButtonStyle.Success);

  const reject = new ButtonBuilder()
    .setCustomId(`contract_reject_${draft.captainDiscordId}_${player.discordId}`)
    .setLabel('RIFIUTA')
    .setStyle(ButtonStyle.Danger);

  const embed = new EmbedBuilder()
    .setColor(0xd4af37)
    .setTitle('📩 Deposito contratto RPCI')
    .setDescription(
      `Sei stato iscritto alla competizione **RPCI** dal club **${draft.clubName}**.\n\n` +
      `Devi decidere se depositare il contratto.`
    )
    .addFields(
      { name: 'Club', value: draft.clubName, inline: true },
      { name: 'Giocatore', value: player.playerName, inline: true },
      ...
      { name: 'ID Console', value: player.consoleId, inline: true },
      { name: 'Contratto', value: `${player.contractYears} anno/i`, inline: true },
      { name: 'Scadenza', value: '24 ore', inline: true }
    )
    .setFooter({ text: 'RPCI • Conferma contratto' })
    .setTimestamp();
  ...
  await scheduleContractAutoReject(draft.captainDiscordId, player.discordId);
}

async function saveContractDeposit(interaction, draft) {
  const rows = draft.players.map(player => ({
    application_id: draft.applicationId,
    discord_id: player.discordId,
    age: null,
    platform: player.platform,
    platform_id: player.consoleId,
    primary_role: 'DA DEFINIRE',
    secondary_role: 'NO',
    contract_years: Number(player.contractYears),
    player_tier: 'ROOKIE',
    salary_per_year: TIER_SALARIES.ROOKIE,
  ...
  draft.captainDiscordId = interaction.user.id;
  ...
  if (channel) {
    ...
    draft.summaryMessageId = summaryMessage.id;
  }

  for (const player of draft.players) {
    await sendContractDecisionDM(draft, player);
  }
}
...
function buildClubStaffEmbed(clubName, captainDiscordId, logoUrl = null) {
  const embed = new EmbedBuilder()
    .setTitle('🏆 Nuova richiesta iscrizione club')
    .setColor(0xd4af37)
    .setDescription('Un capitano ha registrato un nuovo club e attende approvazione staff.')
    .addFields(
      { name: 'Nome club', value: clubName, inline: false },
      { name: 'Capitano', value: `<@${captainDiscordId}>`, inline: true },
      { name: 'Stato', value: 'In attesa di approvazione', inline: true }
    )
    .setFooter({ text: 'RPCI • Iscrizione club ufficiale' })
    .setTimestamp();
...
async function sendClubWelcomeNews(app) {
  const channel = await client.channels.fetch(CLUB_NEWS_CHANNEL_ID).catch(() => null);
  if (!channel) return;

  const clubName = app.clubs?.name || 'Nuovo club RPCI';
  const logoUrl = app.clubs?.logo_url || null;
  const captainId = app.captain_discord_id;

  const embed = new EmbedBuilder()
    .setTitle('📰 NUOVO CLUB APPROVATO')
    .setColor(0xd4af37)
    .setDescription(
      `Benvenuti su **RPCI**!\n\n` +
      `Il club **${clubName}** entra ufficialmente nel progetto **Real Pro Clubs Italia**.`
    )
    .addFields(
      { name: '🏟️ Club', value: clubName, inline: true },
      { name: '👑 Capitano', value: `<@${captainId}>`, inline: true },
      { name: '✅ Stato', value: 'Club approvato dallo staff RPCI', inline: false }
    )
    .setFooter({ text: 'RPCI • Comunicati ufficiali' })
    .setTimestamp();
...
async function finalizeClubOnlyApplication(interaction, draft) {
  ...
  if (
    ...
    await supabase
      ...
      .update({
        ...
        salary_per_year: getSalaryByTier(offer.player_tier || 'ROOKIE'),
response_status: 'accepted'
      })
...
async function upsertFreeAgentProfile(profile) {
  const payload = {
    ...
    player_tier: profile.player_tier || 'ROOKIE',
    performance_points: profile.performance_points || 0,
    salary_per_year: profile.salary_per_year || TIER_SALARIES.ROOKIE,
...
async function makePlayerFreeAgent(guild, appPlayer, reason = 'contract_expired') {
  ...
  const profile = await upsertFreeAgentProfile({
    ...
    player_tier: appPlayer.player_tier || 'ROOKIE',
    performance_points: appPlayer.performance_points || 0,
    salary_per_year: appPlayer.salary_per_year || TIER_SALARIES.ROOKIE,
  ...
  if (profile?.application_player_id) {
    await supabase
      ...
      .update({
        ...
        is_free_agent: false,
        contract_years: offer.contract_years,
        salary_per_year: offer.salary || getSalaryByTier(profile?.player_tier || 'ROOKIE'),
})
...
function getTierFromPoints(points) {
  ...
  return range?.tier || 'ROOKIE';
}

function getSalaryByTier(tier) {
  return TIER_SALARIES[tier] || TIER_SALARIES.ROOKIE;
}
...
function buildCompetitionClubSelect(draft) {
  ...
  return new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .setPlaceholder('Seleziona squadre da aggiungere')
      .setMinValues(1)
      .setMaxValues(Math.max(1, pageClubs.length))
...
function buildCompetitionComponents(draft) {
  const components = [];
  if ((draft.clubs || []).length > 0) components.push(buildCompetitionClubSelect(draft));
...
async function openCompetitionModal(interaction, type) {
  ...
  const logo = new TextInputBuilder()
    ...
    .setLabel('LOGO URL')
    .setPlaceholder('Incolla URL logo oppure scrivi NO')
    .setStyle(TextInputStyle.Short)
    .setRequired(false);
  ...
  let placeholder = 'opzionale';
  if (type === 'league') placeholder = 'gironi=1;livello=1;promosse=0;retrocesse=3';
  if (type === 'national_cup') placeholder = 'andata_ritorno=SI;finale=secca';
  if (type === 'european_cup') placeholder = 'gironi=4;squadre=4;qualificate=2';

  const settings = new TextInputBuilder()
    .setCustomId('settings')
    .setLabel('IMPOSTAZIONI')
...
async function getApprovedClubsWithApplications() {
  ...
  for (const club of clubs) {
    const app = await getApprovedApplicationByClubId(club.id).catch(() => null);
    result.push({
      ...club,
...
client.on('interactionCreate', async interaction => {
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === 'deposito_contratti') {
        const member = await interaction.guild.members.fetch(interaction.user.id).catch(() => null);

        if (!member?.roles.cache.has(CAPTAIN_ROLE_ID) && !isStaffMember(member)) {
          return interaction.reply({
            content: '❌ Solo capitani e staff possono pubblicare il deposito contratti.',
            flags: MessageFlags.Ephemeral
          });
        }

        return interaction.reply(buildContractDepositPanel());
      }
      ...
      if (interaction.commandName === 'registrati') {
        ...
        if (!member?.roles.cache.has(CAPTAIN_ROLE_ID)) {
          return interaction.editReply('❌ Solo chi ha il ruolo **Capitano** può usare `/registrati`.');
        }
    ...
    if (interaction.isStringSelectMenu()) {
      if (interaction.customId === 'contract_player_count_select') {
        const draft = contractDepositDrafts.get(interaction.user.id);
        if (!draft) {
          return interaction.reply({
            content: '❌ Nessun deposito contratti in corso.',
            flags: MessageFlags.Ephemeral
          });
        }

        draft.totalPlayers = Number(interaction.values[0]);
        draft.currentIndex = 0;
        draft.players = [];

        return interaction.update({
          content: '',
          embeds: [buildContractProgressEmbed(draft)],
          components: [
            buildOpenPlayerContractModalButton(0)
          ]
        });
      }

      if (interaction.customId.startsWith('contract_years_select_')) {
        const draft = contractDepositDrafts.get(interaction.user.id);
        if (!draft) {
          return interaction.reply({
            content: '❌ Nessun deposito contratti in corso.',
        ...
        const index = Number(interaction.customId.replace('contract_years_select_', ''));
        if (!draft.players[index]) {
          return interaction.reply({
            content: '❌ Prima inserisci i dati del giocatore.',
            flags: MessageFlags.Ephemeral
          });
        }

        draft.players[index].contractYears = interaction.values[0];

        if (draft.players.length < draft.totalPlayers) {
          draft.currentIndex = draft.players.length;
          return interaction.update({
            content: '',
            embeds: [buildContractProgressEmbed(draft)],
            components: [
              buildOpenPlayerContractModalButton(draft.currentIndex)
            ]
          });
        }
        ...
        return interaction.update({
          content: '',
          embeds: [buildContractProgressEmbed(draft)],
          components: [buildContractConfirmRow()]
        });
      }
...
draft.targetApplicationId = targetApplication?.id || null;
      ...
      if (interaction.customId === 'offer_player_select') {
        ...
        draft.selectedPlayerTier = player.player_tier || 'ROOKIE';
        draft.contractYears = null;
...
` +
    ...
    if (interaction.isButton()) {
      if (
        ...
        interaction.customId.startsWith('contract_reject_')
      ) {
        const accepted = interaction.customId.startsWith('contract_accept_');
        const parts = interaction.customId.split('_');
        const captainDiscordId = parts[2];
        const playerDiscordId = parts[3];

        if (interaction.user.id !== playerDiscordId) {
          return interaction.reply({
            content: '❌ Solo il giocatore interessato può rispondere a questa richiesta.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        if (!draft) {
          return interaction.reply({
            content: '❌ Richiesta contratto non trovata o scaduta.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        if (!player) {
          return interaction.reply({
            content: '❌ Giocatore non trovato nel deposito contratti.',
            flags: MessageFlags.Ephemeral
          });
        }

        if (player.responseStatus !== 'pending') {
          return interaction.reply({
            content: '⚠️ Hai già risposto a questa richiesta.',
            flags: MessageFlags.Ephemeral
          });
        }

        player.responseStatus = accepted ? 'accepted' : 'rejected';

        const playerContractUpdate = accepted
          ? {
              response_status: player.responseStatus,
              player_tier: 'ROOKIE',
              salary_per_year: TIER_SALARIES.ROOKIE,
              performance_points: 0
            }
          ...
          if (userRow?.id) {
            ...
            if (existingPlayer?.id) {
              ...
              await supabase
                ...
                .insert({
                  user_id: userRow.id,
                  ea_id: player.consoleId,
                  main_role: 'DA DEFINIRE',
                  status: 'free_agent',
                  salary_per_year: 2,
                  player_tier: 'ROOKIE'
                });
        ...
        if (accepted) {
          await sendPlayerOfficialAnnouncement(draft, player);
        }

        return interaction.update({
          content: accepted
            ? '✅ Hai accettato il deposito del contratto RPCI.'
            : '❌ Hai rifiutato il deposito del contratto RPCI.',
          embeds: interaction.message.embeds,
          components: []
        });
      }

      if (interaction.customId === 'deposit_contracts_start') {
        const member = await interaction.guild.members.fetch(interaction.user.id).catch(() => null);
        if (!member?.roles.cache.has(CAPTAIN_ROLE_ID)) {
          return interaction.reply({
            content: '❌ Solo chi ha il ruolo **Capitano** può depositare i contratti.',
            flags: MessageFlags.Ephemeral
          });
        }

        const app = await getCaptainClubForContracts(interaction.user.id);
        if (!app) {
          return interaction.reply({
            content: '❌ Devi avere un club approvato dallo staff prima di depositare i contratti.',
            flags: MessageFlags.Ephemeral
          });
        }

        contractDepositDrafts.set(interaction.user.id, {
          applicationId: app.id,
          captainDiscordId: interaction.user.id,
          clubId: app.club_id,
          clubName: app.clubs?.name || 'Club RPCI',
          clubLogoUrl: app.clubs?.logo_url || null,
          totalPlayers: null,
          currentIndex: 0,
          players: []
        });
        ...
        return interaction.reply({
          content: 'Seleziona quanti giocatori devi iscrivere. **Minimo 5**.',
          components: [buildContractPlayerCountSelect()],
          flags: MessageFlags.Ephemeral
        });
      }

      if (interaction.customId.startsWith('contract_player_modal_')) {
        const draft = contractDepositDrafts.get(interaction.user.id);
        if (!draft) {
          return interaction.reply({
            content: '❌ Nessun deposito contratti in corso.',
        ...
        const index = Number(interaction.customId.replace('contract_player_modal_', ''));
        if (index !== draft.currentIndex) {
          return interaction.reply({
            content: '❌ Devi completare i giocatori in ordine.',
            flags: MessageFlags.Ephemeral
          });
        }

        const modal = new ModalBuilder()
          .setCustomId(`contract_player_modal_submit_${index}`)
          .setTitle(`Giocatore ${index + 1}/${draft.totalPlayers}`);
        ...
        const discordId = new TextInputBuilder()
          ...
          .setLabel('ID DISCORD')
          .setPlaceholder('Esempio: 123456789012345678')
          .setStyle(TextInputStyle.Short)
          .setRequired(true);

        const platform = new TextInputBuilder()
          .setCustomId('platform')
          .setLabel('CONSOLE: PS5 / XBOX / PC')
          .setPlaceholder('Scrivi PS5, XBOX oppure PC')
          .setStyle(TextInputStyle.Short)
          .setRequired(true);

        const consoleId = new TextInputBuilder()
          .setCustomId('console_id')
          .setLabel('ID CONSOLE')
          .setPlaceholder('Scrivi l’ID console del giocatore')
          .setStyle(TextInputStyle.Short)
          .setRequired(true);
        ...
        modal.addComponents(
          ...
          new ActionRowBuilder().addComponents(consoleId)
        );

        return interaction.showModal(modal);
      }

      if (interaction.customId === 'contract_deposit_confirm') {
        const draft = contractDepositDrafts.get(interaction.user.id);
        if (!draft) {
          return interaction.reply({
            content: '❌ Nessun deposito contratti in corso.',
        ...
        if (draft.players.length < 5 || draft.players.length !== draft.totalPlayers) {
          return interaction.reply({
            content: '❌ Devi completare tutti i giocatori prima di inviare.',
            flags: MessageFlags.Ephemeral
          });
        }

        await saveContractDeposit(interaction, draft);

        return interaction.update({
          content: `✅ Contratti depositati correttamente per **${draft.clubName}**.\n\nGiocatori inseriti: **${draft.players.length}**.`,
          embeds: [],
          components: []
        });
      }

      if (interaction.customId === 'contract_deposit_cancel') {
        contractDepositDrafts.delete(interaction.user.id);
        return interaction.update({
          content: '❌ Deposito contratti annullato.',
          embeds: [],
          components: []
        });
      }

      if (interaction.customId === 'become_captain') {
        if (interaction.channelId !== CAPTAIN_PANEL_CHANNEL_ID) {
          return interaction.reply({
            content: '❌ Puoi usare questo pulsante solo nel canale DIVENTA CAPITANO.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        if (!member) {
          return interaction.reply({
            content: '❌ Non riesco a trovare il tuo profilo nel server.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        if (member.roles.cache.has(CAPTAIN_ROLE_ID)) {
          return interaction.reply({
            content: '⚠️ Sei già un **Capitano RPCI**.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        return interaction.reply({
          content:
            '✅ **Ora sei un Capitano RPCI**\n\n' +
            'Ora puoi iscrivere la tua squadra su questo canale 📋│ɪꜱᴄʀɪᴢɪᴏɴᴇ-ꜱǫᴜᴀᴅʀᴀ',
          flags: MessageFlags.Ephemeral
        });
      }
  ...
  status: 'pending'
      ...
      if (interaction.customId === 'start_registration') {
        ...
        if (!member?.roles.cache.has(CAPTAIN_ROLE_ID)) {
          return interaction.reply({
            content: '❌ Devi prima ottenere il ruolo **Capitano** dal pannello dedicato.',
            flags: MessageFlags.Ephemeral
          });
        }

        let { user: captainUser, player: captainPlayer } = await getOrCreateUserAndPlayer(
          interaction.user
        );

        if (!captainPlayer) {
          const { data: newCaptainPlayer, error: captainPlayerError } = await supabase
            .from('players')
            .insert({
              user_id: captainUser.id,
              ea_id: interaction.user.username,
              main_role: 'CAPITANO',
              status: 'free_agent'
            })
            .select()
          ...
          if (captainPlayerError) throw captainPlayerError;
          captainPlayer = newCaptainPlayer;
        }
      ...
      if (
        ...
        if (accepted) {
          ...
          if (captainMember) {
            await captainMember.roles.add(CAPTAIN_ROLE_ID).catch(() => null);
          }
          ...
          await ensureClubBudget(app.id, app.clubs?.name || 'Club RPCI').catch(console.error);

          await sendClubWelcomeNews(app).catch(console.error);

          const captainUser = await client.users.fetch(app.captain_discord_id).catch(() => null);
          if (captainUser) {
            await captainUser.send(
              `✅ Il tuo club **${app.clubs?.name || 'RPCI'}** è stato approvato dallo staff RPCI.\n\n` +
              `Ora devi procedere con l’iscrizione e depositare i contratti dei tuoi giocatori in rosa nel canale:\n` +
              `📩│ᴅᴇᴘᴏꜱɪᴛᴏ-ᴄᴏɴᴛʀᴀᴛᴛɪ\n` +
              `https://discord.com/channels/1396908861566353479/1508454065535979591\n\n` +
              `Segui attentamente le istruzioni che trovi lì.`
            ).catch(() => null);
          }
        }
        ...
        return interaction.update({
          content: accepted
            ? '✅ Club approvato dallo staff.'
            : '❌ Club rifiutato dallo staff.',
          embeds: interaction.message.embeds,
          components: []
        });
      }
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId.startsWith('contract_player_modal_submit_')) {
        const draft = contractDepositDrafts.get(interaction.user.id);
        if (!draft) {
          return interaction.reply({
            content: '❌ Nessun deposito contratti in corso.',
        ...
        const index = Number(interaction.customId.replace('contract_player_modal_submit_', ''));
        ...
        const platformRaw = interaction.fields.getTextInputValue('platform').trim().toUpperCase();
        const consoleId = interaction.fields.getTextInputValue('console_id').trim();

        if (!/^\d{16,22}$/.test(discordId)) {
          return interaction.reply({
            content: '❌ ID Discord non valido. Devi inserire solo il numero ID Discord del giocatore.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        if (!guildMember) {
          return interaction.reply({
            content: '❌ L’ID Discord inserito non corrisponde a un membro presente nel server.',
            flags: MessageFlags.Ephemeral
          });
        }

        if (!['PS5', 'XBOX', 'PC'].includes(platformRaw)) {
          return interaction.reply({
            content: '❌ Console non valida. Devi scrivere **PS5**, **XBOX** oppure **PC**.',
            flags: MessageFlags.Ephemeral
          });
        }

        if (draft.players.some(p => p.discordId === discordId)) {
          return interaction.reply({
            content: '❌ Questo giocatore è già stato inserito nel deposito.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        return interaction.reply({
          content: `✅ Dati giocatore **${playerName}** salvati.\n\nOra seleziona gli anni di contratto.`,
          components: [buildContractYearsSelect(index)],
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (interaction.customId === 'looking_description_modal') {
        ...
        const profile = await upsertFreeAgentProfile({
          ...draft,
          secondary_role: draft.secondary_role || 'NO',
          player_tier: currentClub?.row?.player_tier || 'ROOKIE',
          performance_points: currentClub?.row?.performance_points || 0,
          salary_per_year: currentClub?.row?.salary_per_year || TIER_SALARIES.ROOKIE,
      ...
      if (interaction.customId === 'deposit_salary_modal_submit') {
        ...
        const minSalary = getSalaryByTier(draft.selectedPlayer?.player_tier || 'ROOKIE');
        if (salary < minSalary) {
          return interaction.reply({ content: `❌ Stipendio troppo basso. Minimo per fascia **${draft.selectedPlayer?.player_tier || 'ROOKIE'}**: **${minSalary} crediti/anno**.`, flags: MessageFlags.Ephemeral });
        }
      ...
      if (interaction.customId === 'team_modal') {
        ...
        try {
          ...
          await finalizeClubOnlyApplication(interaction, draft);

          return interaction.followUp({
            content:
              '✅ Iscrizione club inviata correttamente allo staff RPCI.\n\n' +
              'Riceverai conferma quando il club verrà approvato.',
            flags: MessageFlags.Ephemeral
          });
