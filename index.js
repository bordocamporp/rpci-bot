...
async function openCompetitionModal(interaction, type) {
  ...
  const nation = new TextInputBuilder()
    ...
    .setPlaceholder(type === 'european_cup' ? 'Europa' : 'Italia')
    .setStyle(TextInputStyle.Short)
    .setRequired(false);
...
function buildCompetitionLogoStepEmbed(draft) {
  const embed = new EmbedBuilder()
    .setTitle('🖼️ Logo competizione')
    .setColor(0xd4af37)
    .setDescription(
      `Competizione: **${draft.name}**\n\n` +
      'Premi **CARICA LOGO** per inviare il file immagine nel canale, oppure **SALTA LOGO** se non vuoi inserirlo ora.'
    )
    .setFooter({ text: 'RPCI • Logo competizione' })
    .setTimestamp();

  if (draft.logoUrl) embed.setThumbnail(draft.logoUrl);
  return embed;
}

function buildCompetitionLogoStepButtons() {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId('competition_upload_logo')
      .setLabel('CARICA LOGO')
      .setStyle(ButtonStyle.Primary),
    new ButtonBuilder()
      .setCustomId('competition_skip_logo')
      .setLabel('SALTA LOGO')
      .setStyle(ButtonStyle.Secondary),
    new ButtonBuilder()
      .setCustomId('competition_cancel')
...
function buildNumberSelect(customId, placeholder, min, max, selectedValue = null) {
  const options = [];
  for (let i = min; i <= max; i++) {
    options.push({
      label: String(i),
      value: String(i),
      default: Number(selectedValue) === i
    });
  }
...
function buildCompetitionSettingsEmbed(draft) {
  const settings = draft.settings || {};
  const description = draft.type === 'league'
    ? 'Scegli le impostazioni del campionato dai menù qui sotto. Il vecchio campo **livello** è stato rimosso: non serve per creare il calendario.'
    : draft.type === 'european_cup'
      ? 'Scegli le impostazioni della coppa europea dai menù qui sotto.'
      : 'Coppa nazionale: tabellone con andata/ritorno e finale secca.';

  return new EmbedBuilder()
    .setTitle('⚙️ Impostazioni competizione')
    .setColor(0xd4af37)
    .setDescription(description)
    .addFields(
      { name: 'Gironi', value: String(settings.gironi ?? (draft.type === 'european_cup' ? 4 : 1)), inline: true },
      { name: 'Promosse', value: String(settings.promosse ?? 1), inline: true },
      { name: 'Retrocesse', value: String(settings.retrocesse ?? 1), inline: true }
    )
    .setFooter({ text: 'RPCI • Impostazioni competizione' })
    .setTimestamp();
}

function buildCompetitionSettingsComponents(draft) {
  const settings = draft.settings || {};

  if (draft.type === 'national_cup') {
    return [
      new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId('competition_settings_continue').setLabel('CONTINUA').setStyle(ButtonStyle.Success),
  ...
  return [
    buildNumberSelect('competition_settings_gironi', 'Numero gironi (1-4)', 1, 4, settings.gironi),
    buildNumberSelect('competition_settings_promosse', 'Promosse (1-8)', 1, 8, settings.promosse),
    buildNumberSelect('competition_settings_retrocesse', 'Retrocesse (1-8)', 1, 8, settings.retrocesse),
    new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('competition_settings_continue').setLabel('CONTINUA').setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId('competition_cancel').setLabel('ANNULLA').setStyle(ButtonStyle.Danger)
    )
...
async function showCompetitionSettingsStep(interaction, draft, mode = 'reply') {
  const payload = {
    embeds: [buildCompetitionSettingsEmbed(draft)],
    components: buildCompetitionSettingsComponents(draft),
    flags: MessageFlags.Ephemeral
  };

  if (mode === 'update') return interaction.update({ embeds: payload.embeds, components: payload.components, content: '' });
  if (mode === 'edit') return interaction.editReply({ embeds: payload.embeds, components: payload.components, content: '' });
  if (mode === 'followUp') return interaction.followUp(payload);
  return interaction.reply(payload);
}

async function showCompetitionClubStep(interaction, draft, mode = 'update') {
  const clubs = await getApprovedClubsWithApplications();
  if (clubs.length < 2) {
    competitionDrafts.delete(interaction.user.id);
    const payload = { content: '❌ Servono almeno 2 club approvati per creare una competizione.', embeds: [], components: [], flags: MessageFlags.Ephemeral };
  ...
  draft.clubs = clubs;
  draft.clubPage = 0;
  draft.selectedClubIds = [];

  const payload = {
    content: '✅ Impostazioni salvate. Ora seleziona le squadre partecipanti.',
    embeds: [buildCompetitionEmbed(draft)],
    components: buildCompetitionComponents(draft),
    flags: MessageFlags.Ephemeral
  };

  if (mode === 'edit') return interaction.editReply(payload);
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    // STEP 1 COMPETIZIONI
    // =========================
    if (interaction.isStringSelectMenu() && interaction.customId === 'competition_type_select') {
      const member = await interaction.guild.members.fetch(interaction.user.id);
      if (!isStaffMember(member)) {
      ...
      if (!['league', 'national_cup', 'european_cup'].includes(type)) {
        return interaction.reply({ content: '❌ Tipo competizione non valido.', flags: MessageFlags.Ephemeral });
      }

      return openCompetitionModal(interaction, type);
    }

    if (interaction.isModalSubmit() && interaction.customId.startsWith('competition_modal_')) {
      ...
      const nation = interaction.fields.getTextInputValue('nation')?.trim() || '';

      if (!name || !season) {
        return interaction.reply({ content: '❌ Nome e stagione sono obbligatori.', flags: MessageFlags.Ephemeral });
      }

      const defaultSettings = type === 'league'
        ? { gironi: 1, promosse: 1, retrocesse: 1 }
        : type === 'european_cup'
          ? { gironi: 4, promosse: 1, retrocesse: 1, squadre_per_girone: 4, qualificate_per_girone: 2 }
          : { andata_ritorno: 'SI', finale: 'secca' };
      ...
      return interaction.reply({
        embeds: [buildCompetitionLogoStepEmbed(draft)],
        components: [buildCompetitionLogoStepButtons()],
        flags: MessageFlags.Ephemeral
      });
    }

    if (interaction.isButton() && interaction.customId === 'competition_upload_logo') {
      const draft = competitionDrafts.get(interaction.user.id);
      if (!draft) return interaction.reply({ content: '❌ Nessuna creazione competizione in corso.', flags: MessageFlags.Ephemeral });

      await interaction.reply({
        content:
          '📎 Carica ora il **logo della competizione** nel canale.\n' +
          'Puoi inviare PNG/JPG/WebP. Hai 2 minuti di tempo.',
        flags: MessageFlags.Ephemeral
      });
      ...
      try {
        ...
        const message = collected.first();
        const file = message.attachments.first();
        const valid = /\.(png|jpg|jpeg|webp)$/i.test(file.name || '');

        if (!valid) {
          return interaction.followUp({
            content: '❌ Formato logo non valido. Usa PNG, JPG o WebP.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        return showCompetitionSettingsStep(interaction, draft, 'followUp');
      } catch {
        return interaction.followUp({
          content: '❌ Tempo scaduto per il caricamento logo. Puoi ripartire con `/crea_competizione` oppure usare SALTA LOGO.',
          flags: MessageFlags.Ephemeral
        });
      }
    }

    if (interaction.isButton() && interaction.customId === 'competition_skip_logo') {
      const draft = competitionDrafts.get(interaction.user.id);
      if (!draft) return interaction.reply({ content: '❌ Nessuna creazione competizione in corso.', flags: MessageFlags.Ephemeral });
      draft.logoUrl = null;
      return showCompetitionSettingsStep(interaction, draft, 'update');
    }

    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('competition_settings_')) {
      const draft = competitionDrafts.get(interaction.user.id);
      if (!draft) return interaction.reply({ content: '❌ Nessuna creazione competizione in corso.', flags: MessageFlags.Ephemeral });

      const value = Number(interaction.values[0]);
      if (!Number.isInteger(value)) return interaction.reply({ content: '❌ Valore non valido.', flags: MessageFlags.Ephemeral });

      if (interaction.customId === 'competition_settings_gironi') draft.settings.gironi = value;
      if (interaction.customId === 'competition_settings_promosse') draft.settings.promosse = value;
      if (interaction.customId === 'competition_settings_retrocesse') draft.settings.retrocesse = value;
    ...
    if (interaction.isButton() && interaction.customId === 'competition_settings_continue') {
      const draft = competitionDrafts.get(interaction.user.id);
      if (!draft) return interaction.reply({ content: '❌ Nessuna creazione competizione in corso.', flags: MessageFlags.Ephemeral });
      return showCompetitionClubStep(interaction, draft, 'update');
    }

    if (interaction.isStringSelectMenu() && interaction.customId === 'competition_club_select') {
      const draft = competitionDrafts.get(interaction.user.id);
      if (!draft) return interaction.reply({ content: '❌ Nessuna creazione competizione in corso.', flags: MessageFlags.Ephemeral });

      const ids = interaction.values || [];
      draft.selectedClubIds = Array.from(new Set([...(draft.selectedClubIds || []), ...ids]));

      return updateCompetitionMessage(interaction, draft);
    }

    if (interaction.isButton() && (interaction.customId === 'competition_prev_page' || interaction.customId === 'competition_next_page')) {
      const draft = competitionDrafts.get(interaction.user.id);
      if (!draft) return interaction.reply({ content: '❌ Nessuna creazione competizione in corso.', flags: MessageFlags.Ephemeral });

      const totalPages = Math.max(1, Math.ceil((draft.clubs || []).length / COMPETITION_CLUBS_PER_PAGE));
      if (interaction.customId === 'competition_prev_page') draft.clubPage = Math.max(0, (draft.clubPage || 0) - 1);
      else draft.clubPage = Math.min(totalPages - 1, (draft.clubPage || 0) + 1);

      return updateCompetitionMessage(interaction, draft);
    }

    if (interaction.isButton() && interaction.customId === 'competition_cancel') {
      competitionDrafts.delete(interaction.user.id);
      return interaction.update({ content: '❌ Creazione competizione annullata.', embeds: [], components: [] });
    }

    if (interaction.isButton() && interaction.customId === 'competition_confirm') {
      const draft = competitionDrafts.get(interaction.user.id);
      if (!draft) return interaction.reply({ content: '❌ Nessuna creazione competizione in corso.', flags: MessageFlags.Ephemeral });
      if (!draft.selectedClubIds || draft.selectedClubIds.length < 2) {
      ...
      return interaction.editReply({
        content: `✅ Competizione **${competition.name}** creata correttamente con **${draft.selectedClubIds.length}** squadre.\nOra puoi usare \`/genera_calendario\`.`,
        embeds: [],
        components: []
      });
    }
